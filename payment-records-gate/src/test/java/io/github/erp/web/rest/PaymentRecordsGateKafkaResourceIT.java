...
class PaymentRecordsGateKafkaResourceIT {
    ...
    void setup() {
        ...
        PaymentRecordsGateKafkaResource kafkaResource = new PaymentRecordsGateKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/payment-records-gate-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/payment-records-gate-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
