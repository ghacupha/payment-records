...
describe('Placeholder e2e test', () => {
  ...
  it('should create and save Placeholders', async () => {
    ...
    await promise.all([placeholderUpdatePage.setDescriptionInput('description'), placeholderUpdatePage.setTokenInput('token')]);

    expect(await placeholderUpdatePage.getDescriptionInput()).to.eq(
      'description',
      'Expected Description value to be equals to description'
    );
    expect(await placeholderUpdatePage.getTokenInput()).to.eq('token', 'Expected Token value to be equals to token');
