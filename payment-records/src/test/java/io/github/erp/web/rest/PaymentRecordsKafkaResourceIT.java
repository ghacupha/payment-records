...
class PaymentRecordsKafkaResourceIT {
    ...
    void setup() {
        ...
        PaymentRecordsKafkaResource kafkaResource = new PaymentRecordsKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/payment-records-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/payment-records-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
